...
\begin{titlepage}
  \thispagestyle{empty}
	...
	\begin{tikzpicture}[remember picture,overlay]
		...
		\fill[white!15!MainBlue, opacity=0.8] ($(current page.north west)+(1cm,-4cm)$) rectangle ($(current page.south east)+(-1cm,-12cm)$);
		
		% خطوط تزئینی
		\draw[AccentGold, line width=3pt] ($(current page.north west)+(1.5cm,-4.5cm)$) -- ($(current page.north east)+(-1.5cm,-4.5cm)$);
		\draw[AccentGold, line width=3pt] ($(current page.north west)+(1.5cm,-14cm)$) -- ($(current page.north east)+(-1.5cm,-14cm)$);
	\end{tikzpicture}
	
	\vspace*{4.5cm}
	
  {\Huge\bfseries\color{white}\shadow{\Huge طرح نهادی گذار ایران}}
	...
	\begin{tcolorbox}[
		...
		\vskip 0.3cm
		\large\mdseries
		به هم آمیزی هر دو بال‌ تکنوکراسی و دموکراسی برای نیل به ایرانی آزاد و آباد
	\end{tcolorbox}
...
\begin{summarybox}
	این سند چارچوب نهادی پیشنهادی برای دوران گذار دموکراتیک ایران را تشریح
	می‌کند. محور آن \textbf{مجلس مهستان} — یک نهاد منتخب و مردم‌محور — است که مسئولیت
	هدایت فرایند گذار را بر عهده می‌گیرد. این طرح بر اساس اصولی استوار است که هدف آن‌ها تضمین انتقال قدرت صلح‌آمیز، حفظ حقوق و آزادی‌های شهروندان، و ایجاد پایه‌ای محکم برای دموکراسی پایدار است.
	
	این سند زنده و پویا است. تمام پیشنهادها، اعداد و ساختارها برای بحث و چکش‌کاری آماده‌اند.
	تصمیم نهایی درباره‌ی آینده‌ی ایران فقط و فقط با \textbf{مردم ایران} است. با این وجود برخی نکات در این طرح با اندیشه و مبتنی بر دروس آموخته‌ی تخصصی و تجارب جهانی شکل گرفته است که نویسنده بر آن‌ها اصرار دارد. به شکلی که تغییر و یا حذف آن‌ها مورد تایید او نخواهد بود. این موارد در بخش جداگانه‌ای تشریح شده است. 

    طبیعی است، در مقاطعی از این دست، ملت‌ها با یک دوگانه‌ی مهم و اساسی روبرو هستند، (۱) آیا باید به نسل‌های آتی حق تعیین سرنوشت بدهند و آزادی انتخاب آن‌ها را محترم بشمارند؟ یعنی کمتر از اصول و قواعد تخلف ناپذیر استفاده کنند؟ (با توجه به این مهم که اصول تخلف ناپذیر در حول و حوش خود، نهادهایی و روابط قدرتی شکل خواهند داد که تغییر آن جز با خشونت و درگیری میسر شدنی نیست!) یا (۲) باید بر اساس تجاربی که کسب کرده‌اند و بر اساس ضرورت‌هایی که وجود دارند، یک نقشه‌ی راه محکم و بنیادی اصولی برای آیندگان به یادگار بگذارند، چرا که ممکن است هیجاناتی و یا توهماتی منجر به شکست‌هایی فاجعه‌بار، هزینه‌هایی غیر ضروری و تکرار تجارب تلخی باشد که هر آنچه برای آن زحمت کشیده شده است در خلال آن زایل شوند! 
    \\آیا تعیین اصول تخلف ناپذیر و یا سخت برای تغییر، تعیین تکلیف و پدرسالاری برای آیندگان است و یا یک تصمیم هوشمندانه و میراثی گهربار که از رواداری و آزادی و تکثر میان آن‌ها محافظت خواهد کرد؟ 
...
عقیده‌محوری &  & اندیشه‌محوری \\
تکلیف محوری &  & حق محوری \\
...
استبداد و خود رایی محوری &  & هم‌فهمی و گفتگو محوری \\
