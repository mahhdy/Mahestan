...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \fill[MahestanDarkBlue] (current page.north west) rectangle
    ([yshift=-7cm]current page.north east);
  % Gold accent band
  \fill[MahestanGold] ([yshift=-7cm]current page.north west) rectangle
    ([yshift=-7.5cm]current page.north east);
  % Light background bottom
  \fill[MahestanCream] ([yshift=-7.5cm]current page.north west) rectangle
    (current page.south east);
  % Decorative circle
  \draw[MahestanGold!40, line width=2pt]
    ([xshift=-3cm, yshift=-3.5cm]current page.north east) circle (3.5cm);
  \draw[MahestanGold!20, line width=4pt]
    ([xshift=-3cm, yshift=-3.5cm]current page.north east) circle (4.2cm);
\end{tikzpicture}

\vspace*{1cm}
\begin{center}
  {\color{white}\small\lr{Version 2.0 — February 2026}}\\[0.5cm]
  {\color{MahestanGold!80}\Large طرح راهنمای گذار}\\[0.3cm]
  \vspace{0.8cm}
  {\color{white}\fontsize{36}{44}\selectfont\bfseries
   بیانیه‌ی چارچوب نهادی گذار ایران}\\[0.5cm]
  {\color{MahestanLightBlue}\fontsize{22}{30}\selectfont
   طرح مجلس مهستان}\\[0.3cm]
  {\color{MahestanGold}\rule{12cm}{2pt}}
  \vspace{4.5cm}
  ...
  \begin{center}
    {\large\bfseries\color{MahestanDarkBlue}
     نسخه‌ی بازنگری‌شده}\\[0.4cm]
    {\normalsize\color{MahestanGray}
     این سند چارچوب پیشنهادی نهادی برای دوران گذار ایران است.}\\[0.2cm]
    {\normalsize\color{MahestanGray}
     تمام اعداد و نسبت‌ها پیشنهادی برای بحث بوده و توسط}\\[0.1cm]
    {\normalsize\color{MahestanGray}
     مجلس مهستان منتخب مردم نهایی خواهند شد.}\\[0.4cm]
    \hrule\vspace{0.3cm}
    {\small\color{MahestanGray}
     آخرین ویرایش: اسفند ۱۴۰۴ \lr{/} فوریه‌ی ۲۰۲۶ \quad
     \lr{v2.0}}
  \end{center}
  \end{tcolorbox}
\end{center}
